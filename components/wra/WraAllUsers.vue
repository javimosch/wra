<template>
	<div class="WraAllUsers row">
		...
		<div class="col-12" v-show="isList">
			<WraAllUsersList 
			:items="items"
			@select="select" :item.sync="item"></WraAllUsersList>
		</div>
...
<script>
	...
	import WraAllUsersList from '@/components/wra/WraAllUsersList'
	...
	export default {
		...
		methods:{
			...
			async refresh(){
				this.items = await call('wraFetchAllusers');
			},
			select(item){
				this.mode="details";
			...
			async save(back){
				...
				await call('wraSaveUser',this.item)
